...
static t_flags      treat_flags(va_list args, t_flags fl)
{
    ...
    j = 0;
    while (fl.set[j] != '\0' && ft_strchr_ok(FLAGS, fl.set[j]))
    {
        if (fl.set[j++] == '0')
            fl.pad_c = '0';
    }
    ...
    while (fl.set[j] != '\0' && ft_strchr_ok("l", fl.set[j++]))
        fl.length++;
    while (fl.set[j] != '\0' && ft_strchr_ok("h", fl.set[j++]))
        fl.length--;
    return (fl);
}
...
static void         get_fspecs(va_list args, const char *format, int *len, int *i)
{
    ...
    j = 0;
    while (ft_strchr_ok(ALL_FL, format[*i]) && j < 19)
        fl.set[j++] = format[(*i)++];
    fl.set[j] = '\0';
    if (ft_strchr_ok(FSPECS, format[*i]))
    {
        fl.spe_c = format[(*i)++];
        fl.minus = ft_strchr_ok(fl.set, '-');
        fl.plus = ft_strchr_ok(fl.set, '+');
        fl.space = ft_strchr_ok(fl.set, ' ');
        fl.minus = ft_strchr_ok(fl.set, '#');
        fl.width = 0;
        fl.point = 0;
        fl.precision = 0;
